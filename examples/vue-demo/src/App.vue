...
// The host owns the day's jobs: ONE list — the SDK infers service (single
// `location`) vs shipment (`pickup` + `delivery`) from each item's shape.
...
const day = ref(DAY1)
const jobs = computed<PlannerJobInput[]>(() => JOBS_BY_DAY[day.value] ?? [])

function onDayChange(e: Event) {
  day.value = (e as CustomEvent<{ day: string }>).detail.day
}
</script>

<template>
  <!--
    Vue binds objects/arrays onto a custom element as DOM PROPERTIES with the
    `.prop` modifier (`:jobs.prop`), plain strings as attributes (`:day`), and
    listens to its events with `@vm-daychange`. No ref, no imperative wiring.
  -->
